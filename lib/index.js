...
service.interceptors.request.use(
  config => {
    ...
    if (microServices) {
      config.headers[microServices.token] = microServices.tokenPrefix + getToken()
    }
    // 灰度用户标记
...
export function download (url, params) {
  ...
  let headers = {}
  if (microServices) {
    headers[microServices.token] = microServices.tokenPrefix + getToken()
  }
  // 灰度用户标记
  const endUser = getLocalStorage('END_USER')
  if (endUser) {
