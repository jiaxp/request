...
let FN_ERROR, FN_EXCEPT, FN_EXPIRE
...
service.interceptors.request.use(
  config => {
    ...
    if (microServices) {
      config.headers[microServices.token] = microServices.tokenPrefix + getToken(TOKEN_KEY, NAMESPACE)
    }
    // 灰度用户标记
...
export function download (url, params) {
  ...
  let headers = {}
  if (microServices) {
    headers[microServices.token] = microServices.tokenPrefix + getToken(TOKEN_KEY, NAMESPACE)
  }
  // 灰度用户标记
  const endUser = getLocalStorage('END_USER')
  if (endUser) {
  ...
  axios({
    ...
    params: {
      ...params,
      token: getToken(TOKEN_KEY, NAMESPACE)
    },
    responseType: 'blob',
    headers: headers
  }).then(response => {
